...
typedef struct sServerDescription
{
  ...
  uint16_t ConnectionDataPort;              // The data port this server is configures to use.
...
typedef struct sMarkerSetDescription
{
  ...
  int32_t nMarkers;                         // number of markers in MarkerSet
...
typedef struct sRigidBidyDescription
{
...
typedef struct sRigidBodyData
{
  int32_t ID;                               // RigidBody identifier:
                                            ...
                                            // For skeleton assets, this combines both skeleton ID (High-bit) and Bode ID (Low-bit).
...
typedef struct sSkeletonDescription
{
  ...
  int32_t nRigidBodies;                     // number of rigid bodies (bones) skeleton
  sRigidBodyDescription RigidBodies[MAX_SKELRIGIDBODIES]; // array of rigid body (bone) description
...
{
  int32_t skeletonID;                       // Skeleton unique identifier
  int32_t nRigidBodies;                     // number of rigid bodies
...
typedef struct sForcePlateDescription
{
  ...
  int32_t nChannels;                        // number of channels (signals)
