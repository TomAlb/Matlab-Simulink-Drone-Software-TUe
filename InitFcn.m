...
%% Internal Model Principles
% X-Dynamics
Pa = [0 1; 0 0]; Pb = [0; 1]; Pc = [1 0];
P = c2d(ss(Pa, Pb, Pc, 0), 1/fs);
Kx = [0.60 0.9];
% X-IMP
S = [0 1; 0 0];
S = c2d(ss(S, zeros(size(S,1),1), zeros(1,size(S,2)), 0), 1/fs);
% Setup IMP Controller constaints
Sigma = sdpvar(size(S.a,2),size(P.a,1));
Be = sdpvar(size(S.a,2),size(P.c,1));
L = [Sigma*(P.a-P.b*Kx) - S.a*Sigma == Be*P.c, abs(Be(:)) <= 1];
...
Sigma = double(Sigma);
Be = double(Be);
Bs = -Sigma * P.b; S = S.a; Be = Be;
Kv = place(S, Bs, [0.99 0.98]);
K_x = Kx - Kv*Sigma;

%% Store X-dynamics
Kx = Pixhawk_CSC.Parameter({single( Kx),'Kx'} );
Sx  = Pixhawk_CSC.Parameter({single( S),'Sx'} );
Bex = Pixhawk_CSC.Parameter({single( Be),'Bex'} );
Bsx = Pixhawk_CSC.Parameter({single( Bs),'Bsx'} );
Csx = Pixhawk_CSC.Parameter({single( [eye(2)]),'Csx'} );
Dsx = Pixhawk_CSC.Parameter({single( [zeros(2)]),'Dsx'} );
Vsx0 = Pixhawk_CSC.Parameter({single( [0; 0]),'Vsx0'} );
Mx  = Pixhawk_CSC.Parameter({single( C13.Value),'Mx'} );
Kxx = Pixhawk_CSC.Parameter({single( K_x),'Kxx'} );
Kvx = Pixhawk_CSC.Parameter({single( Kv),'Kvx'} );
...
Sigma = sdpvar(size(S.a,2),size(P.a,1));
Be = sdpvar(size(S.a,2),size(P.c,1));
L = [Sigma*(P.a-P.b*Ky) - S.a*Sigma == Be*P.c, abs(Be(:)) <= 1];
...
%% Store Y-dynamics
Ky  = Pixhawk_CSC.Parameter({single( Ky),'Ky'} );
Sy  = Pixhawk_CSC.Parameter({single( S),'Sy'} );
Bey = Pixhawk_CSC.Parameter({single( Be),'Bey'} );
Bsy = Pixhawk_CSC.Parameter({single( Bs),'Bsy'} );
Csy = Pixhawk_CSC.Parameter({single( [eye(2)]),'Csy'} );
Dsy = Pixhawk_CSC.Parameter({single( [zeros(2)]),'Dsy'} );
Vsy0 = Pixhawk_CSC.Parameter({single( [0; 0]),'Vsy0'} );
My  = Pixhawk_CSC.Parameter({single( C13.Value),'My'} );
Kxy = Pixhawk_CSC.Parameter({single( K_x),'Kxy'} );
Kvy = Pixhawk_CSC.Parameter({single( Kv),'Kvy'} );
...
Sigma = sdpvar(size(S.a,2),size(P.a,1));
Be = sdpvar(size(S.a,2),size(P.c,1));
L = [Sigma*(P.a-P.b*Kz) - S.a*Sigma == Be*P.c, abs(Be(:)) <= 1];
...
%% Store Z-dynamics
Sz  = Pixhawk_CSC.Parameter({single( S),'Sz'} );
Bez = Pixhawk_CSC.Parameter({single( Be),'Bez'} );
Bsz = Pixhawk_CSC.Parameter({single( Bs),'Bsz'} );
Csz = Pixhawk_CSC.Parameter({single( [eye(2)]),'Csz'} );
Dsz = Pixhawk_CSC.Parameter({single( [zeros(2)]),'Dsz'} );
Vsz0 = Pixhawk_CSC.Parameter({single( [0; 0]),'Vsz0'} );
Mz  = Pixhawk_CSC.Parameter({single( C13.Value),'Mz'} );
Kxz = Pixhawk_CSC.Parameter({single( K_x),'Kxz'} );
Kvz = Pixhawk_CSC.Parameter({single( Kv),'Kvz'} );
