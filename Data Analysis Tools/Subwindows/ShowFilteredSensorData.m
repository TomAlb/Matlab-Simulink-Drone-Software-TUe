...
% SHOWFILTEREDSENSORDATA MATLAB code for showfilteredsensordata.fig
%      SHOWFILTEREDSENSORDATA, by itself, creates a new SHOWFILTEREDSENSORDATA or raises the existing
%      singleton*.
%
%      H = SHOWFILTEREDSENSORDATA returns the handle to a new SHOWFILTEREDSENSORDATA or the handle to
%      the existing singleton*.
%
%      SHOWFILTEREDSENSORDATA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SHOWFILTEREDSENSORDATA.M with the given input arguments.
%
%      SHOWFILTEREDSENSORDATA('Property','Value',...) creates a new SHOWFILTEREDSENSORDATA or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before showfilteredsensordata_openingfcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to showfilteredsensordata_openingfcn via varargin.
%
...
% Edit the above text to modify the response to help showfilteredsensordata

% Last Modified by GUIDE v2.5 20-Aug-2018 14:44:11
...
% --- Executes just before showfilteredsensordata is made visible.
function ShowRAwSensorData_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to showfilteredsensordata (see VARARGIN)

% Choose default command line output for showfilteredsensordata
...
set(handles.SonarRadioBtn, 'value', 0);

cla(handles.FilteredDataAxes, 'reset')
grid on
axis tight
xlabel('Time')
ylabel('Data')
...
% UIWAIT makes showfilteredsensordata wait for user response (see UIRESUME)
% uiwait(handles.ShowFilteredSensorData);
...
Data = handles.MainData.DataSet.FILTERED_SENSOR_DATA(handles.RequestDataFlags, :);
cla(handles.FilteredDataAxes, 'reset')
...
handles.FilteredDataAxes.Units = 'normalized';
handles.Sensor_Panel.Units = 'normalized';
handles.Direction_Panel.Units = 'normalized';

handles.FilteredDataAxes.OuterPosition  = [0 0 0.8 1];
